...
const mov = await fetchJson(`${apilink}/movie/baiscope/search?q=${q}`)
var ty = ''

if (mov.result.data.length < 1) return await reply(not_fo)

let cot = `🔠 *𝖲𝖠𝖭𝖨𝖩𝖠 𝖬𝖣 𝖲𝖴𝖡𝖳𝖨𝖳𝖫𝖤 𝖣𝖮𝖶𝖭𝖫𝖮𝖠𝖣 𝖲𝖸𝖲𝖳𝖤𝖬* 🔠
...
const mov = await fetchJson(`${apilink}/movie/baiscope/dl?url=${q}`)
...
const mov = await fetchJson(`${apilink}/movie/baiscope/dl?url=${inp}`)
